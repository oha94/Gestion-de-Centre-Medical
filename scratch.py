import sys, re
with open(r'c:\Projets\CentreMedical\src\views\DocumentsMain.tsx', 'r', encoding='utf-8') as f:
    content = f.read()

new_func = """function ReportMouvements({ onBack }: { onBack: () => void }) {
...
pattern = re.compile(r'function ReportMouvements\(.*?\).*?// 2. Invendus', re.DOTALL)
if pattern.search(content):
    content = pattern.sub(new_func + '\n\n// 2. Invendus', content)
    with open(r'c:\Projets\CentreMedical\src\views\DocumentsMain.tsx', 'w', encoding='utf-8') as f:
        f.write(content)
    print('SUCCESS')
else:
